...
public class FilterPostsAPI {
    ...
    public void findUserInReceive(int postId, int userId, final UserInReceiveCallback callback) {
        // Tham chiếu đến nút postId trong FilterPosts
        DatabaseReference postRef = receivePostDatabase.child(String.valueOf(postId)).child("Receive");
    ...
    public void addReceivePost(FilterPost receivePost) {
        String postKey = String.valueOf(receivePost.getPostId());

        // Lưu postId trước
        receivePostDatabase.child(postKey).child("postId").setValue(receivePost.getPostId())
        ...
        if (listUserGet != null) {
            for (int i = 0; i < listUserGet.size(); i++) {
                receivePostDatabase.child(postKey).child("Receive").child(String.valueOf(i)).child("userId")
